#include "Sorting.hpp"
std::string slug = "";
bool TradeFrenquency(std::optional<int> rank){
    json stats = GETjson("https://api.warframe.market/v1/items/" + (std::string)slug  + "/statistics", {"accept: application/json", "Language: en"});
    int vol = 0;
...
bool RankBasedMargin(json orders){
    ...
    if(margin > 10 && TradeFrenquency({best_margin_key})){
...
bool CheckMargin(std::string item){
    slug = item;
    json orders = GETjson("https://api.warframe.market/v2/orders/item/" + (std::string)slug, {"accept: application/json", "Language: en"});
    if(orders["data"][0].find("rank") != orders["data"][0].end()){
        if(RankBasedMargin(orders)){
            return true;
        }
        else{
            return false;
        ...
        if(sell - buy > 10 && (sell_trade && buy_trade) && TradeFrenquency()){
